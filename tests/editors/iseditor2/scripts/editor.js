/***********************************************************
InnovaStudio WYSIWYG Editor 2.6
...
function InnovaEditor(oName)
	...
	this.replaceAllFontsWithSpan=replaceAllFontsWithSpan;
	this.removeUnusedSpan=removeUnusedSpan;
	...
	this.iconOffsetTop;//not used (for compatibility with moz.version)
	...
	this.buttonMap=["Save","FullScreen","Preview","Print","Search","SpellCheck","|",
			...
			"Form","RemoveFormat","Clean","HTMLFullSource","HTMLSource","XHTMLFullSource",
	...
	this.btnForm=true;this.btnRemoveFormat=true;this.btnClean=true;
	...
	for(var i=0;i<this.buttonMap.length;i++)
		...
		switch(sButtonName)
			...
			case "Clean":
				if(this.btnClean)
					{
					sHTMLIcons+=this.writeIconStandard("btnClean"+this.oName,this.oName+".dropShow(this,dropClean"+this.oName+")","btnClean.gif",getText("Clean"));

					sHTMLDropMenus+="<table id=dropClean"+this.oName+" cellpadding=0 cellspacing=0 "+
						"style='z-index:1;display:none;position:absolute;border:#80788D 1px solid;"+
						"cursor:default;background-color:#fbfbfd;' unselectable=on>"+
						"<tr><td onclick=\"dropClean"+this.oName+".style.display='none';if("+this.oName+".replaceAllFontsWithSpan())alert(getText('All FONTs removed.'));else alert(getText('No FONTs.'));\" "+
							"style=\"padding:2px;padding-top:1px;font-family:Tahoma;font-size:11px;color:black;\" "+
							"onmouseover=\"this.style.backgroundColor='#708090';this.style.color='#FFFFFF';\" "+
							"onmouseout=\"this.style.backgroundColor='';this.style.color='#000000';\" unselectable=on>"+getText("Remove Font Tags")+"</td></tr>"+
						"<tr><td onclick=\"dropClean"+this.oName+".style.display='none';if("+this.oName+".removeUnusedSpan())alert(getText('All unused SPANs removed.'));else alert(getText('No unused SPANs.'));\" "+
							"style=\"padding:2px;padding-top:1px;font-family:Tahoma;font-size:11px;color:black;\" "+
							"onmouseover=\"this.style.backgroundColor='#708090';this.style.color='#FFFFFF';\" "+
							"onmouseout=\"this.style.backgroundColor='';this.style.color='#000000';\" unselectable=on>"+getText("Remove Unused Spans")+"</td></tr>"+
						"</table>";
					}
				break;
			case "HTMLFullSource":
...
function loadHTML(sHTML)//hanya utk first load.
	...
	this.styleSelHeight=oEditor.document.body.offsetHeight;
	
	this.replaceAllFontsWithSpan()
	}
function doOnPaste()
	{
...
*/
	...
	var arrValue = eval("["+sTmp.substr(1)+"]"); 
	this.arrStyle=arrValue;
	/*** end ***/
...
function ApplyExternalStyle(oName)
	...
	if(arrStyle.length!=0)eval(oName).isCssLoaded=true;//if => pengamanan terakhir
	}
...
function openStyleSelect()
	...
	var oEditor=eval("idContent"+this.oName);	
	var h=this.styleSelHeight-25;
	
	var arrStyle=this.arrStyle;
	var sHTML="";
	sHTML+="<div unselectable=on style='margin:5px;margin-top:0;' align=right>"
	...
	sHTML+="<div unselectable=on style='overflow:auto;height:"+h+"px;'><table cellpadding=0 cellspacing=0 style='background:#fcfcfc;height:100%;'>";
	...
	for(var i=0;i<arrStyle.length;i++)
		...
		if(isOnSelection)
			...
			if(sSelector.split(".").length>1)//sudah pasti
				{
				bShowStyles=true;
				sHTML+="<tr style=\"cursor:default\" onmouseover=\"this.style.background='#708090';this.style.color='white'\" onmouseout=\"this.style.background='';this.style.color=''\">";
				sHTML+="<td unselectable=on onclick=\"doApplyStyle('"+this.oName+"','"+sSelector.split(".")[1]+"')\" style='padding:2px'>";
				if(sSelector.split(".")[0]=="")
					sHTML+="<span unselectable=on style=\""+arrStyle[i][3]+"\">"+sCaption+"</span>";
				else
					sHTML+="<span unselectable=on class=\""+sSelector.split(".")[1]+"\">"+sSelector+"</span>";
				sHTML+="</td></tr>";
				}
			}
		}
...
function applyBold()
	...
	var oEditor=eval("idContent"+this.oName);
	var oSel=oEditor.document.selection.createRange();	
	var selBook = oSel.getBookmark();	

	var allStrongs=new Array();
	if (oSel.parentElement().nodeName=="STRONG" && oSel.parentElement().innerText==oSel.text)
		{
		oSel.moveToElementText(oSel.parentElement())
		allStrongs[0]=oSel.parentElement();
		} 
	else 
		{
		allStrongs=oSel.parentElement().getElementsByTagName("STRONG");
		}
	var tempRange = oEditor.document.body.createTextRange();
	var newSpan;

	var count = allStrongs.length;
	while (count > 0) 
		{
		f = allStrongs[0];
		newSpan = oEditor.document.createElement("B");
		newSpan.innerHTML = f.innerHTML;
		f.replaceNode(newSpan);          
		count--;
		}
	oSel.moveToBookmark(selBook);
	oSel.select();
	realTime(this.oName);
	}
function applyLine(sCmd)
	{
	//if(sCmd=="line-through")this.doCmd("Strikethrough");
	//if(sCmd=="underline")this.doCmd("Underline");
	//return;
	...
	if(sCmd=="underline")
		{
		if(oEditor.document.queryCommandState("Strikethrough")&& !oEditor.document.queryCommandState("Underline")) sValue="underline line-through";
		else if(!oEditor.document.queryCommandState("Strikethrough")&& oEditor.document.queryCommandState("Underline")) sValue="";
		else if(oEditor.document.queryCommandState("Strikethrough")&& oEditor.document.queryCommandState("Underline")) sValue="line-through";
		else if(!oEditor.document.queryCommandState("Strikethrough")&& !oEditor.document.queryCommandState("Underline")) sValue="underline";
		}
	else //"line-through"
		{
		if(oEditor.document.queryCommandState("Strikethrough")&& !oEditor.document.queryCommandState("Underline")) sValue="";
		else if(!oEditor.document.queryCommandState("Strikethrough")&& oEditor.document.queryCommandState("Underline")) sValue="underline line-through";
		else if(oEditor.document.queryCommandState("Strikethrough")&& oEditor.document.queryCommandState("Underline")) sValue="underline";
		else if(!oEditor.document.queryCommandState("Strikethrough")&& !oEditor.document.queryCommandState("Underline")) sValue="line-through";
		}
	...
	if(oUtil.activeElement)
		{
		oElement=oUtil.activeElement;
		if(oElement.tagName!="BODY")oElement.style.textDecoration=sValue;
		}
	else if (oSel.parentElement)	
		{
		if(oSel.text=="")
	...
	if(oSel.parentElement)
		...
		for (var i = 0; i<allSpans.length; i++)
			...
			if (oSel.inRange(tempRange))allSpans[i].style.textDecoration=sValue;
			}
		}*/

	this.removeUnusedSpan();
	realTime(this.oName);
	}
	...
	if(oSel.parentElement)
		...
		for (var i = 0; i<allSpans.length; i++)
			...
			if (oSel.inRange(tempRange)) 
				{
				if(sType=="ForeColor")allSpans[i].style.color=sColor;
				else allSpans[i].style.backgroundColor=sColor;
				}
			}
		}
...
function cleanSpanStyle(oSpan)
	{	
	//Text Formatting Related
	if(oSpan.style.fontFamily=="")
		{
		oSpan.style.cssText = oSpan.style.cssText.replace("FONT-FAMILY: ; ","");
		oSpan.style.cssText = oSpan.style.cssText.replace("FONT-FAMILY: ","");
		}
	if(oSpan.style.fontSize=="")
		{
		oSpan.style.cssText = oSpan.style.cssText.replace("FONT-SIZE: ; ","");
		oSpan.style.cssText = oSpan.style.cssText.replace("FONT-SIZE: ","");
		}
	oSpan.style.cssText = oSpan.style.cssText.replace("FONT-WEIGHT: normal; ","");
	oSpan.style.cssText = oSpan.style.cssText.replace("FONT-WEIGHT: normal","");
	oSpan.style.cssText = oSpan.style.cssText.replace("FONT-STYLE: normal; ","");
	oSpan.style.cssText = oSpan.style.cssText.replace("FONT-STYLE: normal","");
	oSpan.style.cssText = oSpan.style.cssText.replace("TEXT-DECORATION: none; ","");
	oSpan.style.cssText = oSpan.style.cssText.replace("TEXT-DECORATION: none","");
	oSpan.style.cssText = oSpan.style.cssText.replace("LETTER-SPACING: 0px; ","");
	oSpan.style.cssText = oSpan.style.cssText.replace("LETTER-SPACING: 0px","");
	}
	...
	while (count > 0) 
		...
		if (oSel.inRange(tempRange)) 
			...
			*************************************************/
			...
			if(f.parentElement.parentElement.parentElement)
				{
				if (f.parentElement.parentElement.parentElement.nodeName == "SPAN" && f.parentElement.parentElement.parentElement.innerText==f.innerText) 
					{
					newSpan = f.parentElement.parentElement.parentElement; 
			...
			if(f.parentElement.parentElement)
				{
				if (f.parentElement.parentElement.nodeName == "SPAN" && f.parentElement.parentElement.innerText==f.innerText) 
					{
					newSpan = f.parentElement.parentElement; 
					copyAttribute(newSpan, f);
					f.removeNode(false);
					continue;
					}
			...
			if (f.parentElement.nodeName == "SPAN" && f.parentElement.innerText==f.innerText)//Y
				{
				//Tambahan : f.parentElement.innerText==f.innerText
				//Misal: <span style="...">abc 123 def</span>
				//Kalau tdk ada tambahan ini, waktu kita select 123, seluruh span akan ter-apply (abc 123 def)
				
				newSpan = f.parentElement; 
				copyAttribute(newSpan, f);
				f.removeNode(false);
				continue;
				}
			*/
			...
			{
			//alert("OK 2 - " +f.outerHTML)
			
			/*
			misal: ada 3 paragraph:
			<div>abc 123</div>
			<div>def</div>
			<div>ghi</div>
			Kalau kita select mulai 123 s/d ghi. Apply font pertama tdk masalah, tapi yg kedua error.
			Ternyata waktu apply yg kedua, iterasi di 123 menghasilkan oSel.inRange(tempRange) = false.
			shg masuk ke sini & membuat allFonts[i] = allFonts[i+1]; error.
			Khusus utk kasus khusus ini aku pake try catch aja. Kmd pake proses yg sama.
			*/

			//~~~ Sebenarnya bagian ini apa perlu? ~~~
/*			for (var i=0;i<allFonts.length-1;i++)
				{
				try {allFonts[i] = allFonts[i+1];}
				catch(e){;}
				}*/
			//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
function replaceAllFontsWithSpan()
	{
	var oEditor=eval("idContent"+this.oName);
...
function removeUnusedSpan()
	...
	var emptySpans = []; var reg = /<\s*SPAN\s*>/gi;
	for (var i=0; i<allSpans.length; i++) 
		{
...
function doClean()
	...
	if(oUtil.activeElement)
		...
		if (oSel.parentElement)
			...
			if(oEl.outerHTML==oSel.htmlText || oSel.text=="")
				{
...
function hide()
	...
	if(this.btnClean)eval("dropClean"+this.oName).style.display="none";
...
function writeIconToggle(id,command,img,title)
	...
	sHTML=""+
		...
		"<span style='position:relative'><span unselectable='on' style='position:absolute;clip: rect(0 "+w+"px "+h+"px 0)'>"+
