...
function CheckAuthentication() {
    ...
    if (isset($_SESSION ['MAIN']['userid']) && isset($_SESSION ['MAIN']['ckfinder']) && count($_SESSION ['MAIN']['ckfinder']) > 0 && $_SESSION ['MAIN']['access_testmanager'] > 0)
        return true;
    else
        return false;
}
...
/*
  $bas
  eUrl : the base path used to build the final URL for the resources handled
...
if (isset($_SESSION ['MAIN']['userid']) && isset($_SESSION ['MAIN']['ckfinder']))
    $G_SESSION = $_SESSION ['MAIN'];

if ($G_SESSION['ckfinder']['type'] == 'question') {
    $dir = '/test_files/questions/' . $G_SESSION['ckfinder']['id'];
} elseif ($G_SESSION['ckfinder']['type'] == 'test') {
    $dir = '/test_files/tests/' . $G_SESSION['ckfinder']['id'];
} else {
    die();
}

if (!is_dir('/var/www/kkztest.pp.ua' . $dir)) {
    mkdir('/var/www/kkztest.pp.ua' . $dir);
}


$baseUrl = 'http://kkztest.pp.ua' . $dir . '/';
$baseDir = '/var/www/kkztest.pp.ua' . $dir . '/';
